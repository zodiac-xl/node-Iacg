...
jQuery( document ).ready( function ( $ ) {
	...
	if ( slider_autoslide != 0 ) {
		slider_autoslide_holder = setInterval( function () {
			counter++;
	...
	jQuery( '.cube_button_right, .cube_right_arrow' ).click( function () {
		...
		if ( $( '#slider-1' ).find( 'div:animated' ).length == 0 ) {
			counter++;
	...
	function change_image( src_id ) {
		...
		$( '.info_line' ).find( 'a' ).animate( {opacity : 0}, 0, function () {
			$( '.info_line' ).find( 'a' ).css( 'font-size', 48 );
	...
	function transition_random_move( new_src, old_src ) {
		...
		cubes.each( function () {
			...
			if ( direction_up < 0.333 ) {
				direction_up = -1;
			}
			else if ( direction_up < 0.666 ) {
				direction_up = 0;
			}
			else {
				direction_up = 1
			}
			if ( direction_left < 0.333 ) {
				direction_left = -1;
			}
			else if ( direction_left < 0.666 ) {
				direction_left = 0;
			}
			else {
				direction_left = 1
