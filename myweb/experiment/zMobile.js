...
(function () {
	...
	function ajax( arg ) {
		...
		xhr.open( arg.method, url( arg.url, arg.arg ), arg.async );
	...
	function SlideListPanel( parent, arg ) {
		...
		var parentWidth = parent.offsetWidth,
			...
			// 根据宽度计算最大项术
			maxItems = 1; //初始显示的li数量
		while ( arg.width * maxItems + margin * (maxItems - 2) < 1 ) {
			maxItems += 2; //显示的li为填满parent 初始显示的li数量+2
		}
		maxItems+=2;//影藏的两边
		...
		function adjust() {
			var lay = arg.lay;
			lay && loopSelector( ul, "li", function ( li, i ) {
				var offsetIndex = (i - (maxItems - 1) / 2); //相对中心的偏移位置index      (-3)--(-2)--(-1)--0--1--2--3
				!li.zEmpty && lay( li, {
					index:offsetIndex,
					offset : offsetIndex * (itemWidth + marginWidth) + ul.zLeft, //item相对中心的偏移left
					width : itemWidth
				} );
			} );
		}
		...
		function lay( centerIndex ) {
			...
			loop( maxItems, function ( i ) {
				var targetIndex = i - (maxItems - 1) / 2 + centerIndex,
					c=items[cycleIndex(targetIndex)],
					li=null;
				li=items[cycleIndex(targetIndex)].cloneNode(true);
